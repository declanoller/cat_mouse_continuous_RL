...
class CatMouse_PPO:
	...
	def __init__(self, **kwargs):
		...
		self.fname_base = 'CatMouse_PPO_' + fst.getDateString()
		...
		self.LR_policy = default_kwargs['LR_policy']
		self.LR_value = default_kwargs['LR_value']
		...
		self.clip_grad = default_kwargs['clip_grad']
		self.clip_amount = default_kwargs['clip_amount']
		...
		self.explore_steps = default_kwargs['explore_steps']
	...
	def setup_NN(self):
		...
		self.NN_policy = PPO_policy(	self.N_state_terms,
										self.hidden_size,
										self.N_actions,
										sigma_min=self.sigma_min
										)

		self.NN_policy_old = PPO_policy(	self.N_state_terms,
										self.hidden_size,
										self.N_actions,
										sigma_min=self.sigma_min
										)

		self.NN_value = PPO_value(	self.N_state_terms,
									self.hidden_size
									)
		opt_dict = {
			'adam' : optim.Adam,
		...
		self.optim_policy = opt_dict[self.optim](self.NN_policy.parameters(), lr=self.LR_policy)
		self.optim_value = opt_dict[self.optim](self.NN_value.parameters(), lr=self.LR_value)

		self.NN_policy_old.load_state_dict(self.NN_policy.state_dict())

		for l in self.NN_policy_old.layers:
			l.weight.requires_grad = False
			l.bias.requires_grad = False


	def update_old_policy_NN(self):

		for l_old, l in zip(self.NN_policy_old.layers, self.NN_policy.layers):
			l_old.weight.data = l.weight.data
			l_old.bias.data = l.bias.data
	...
	def episode(self, N_steps, **kwargs):
		...
		for t in range(N_steps):
			...
			mu, sigma = self.NN_policy_old.forward(torch.tensor(s, dtype=torch.float))
			...
			#a_tens = pi_dist.sample()
			noise = OU_noise()
			a_tens = pi_dist.sample() + torch.tensor(noise, dtype=torch.float)

			# Iterate, get r, s_next
			r, s_next, done = self.agent.iterate(a_tens.tolist())

			# Batch buffer append
			s_buffer.append(s)
			a_buffer.append(a_tens.detach())
			r_buffer.append(r)

			# Ep hist stuff
			R_ep_hist.append(r)
			s_ep_hist.append(s)
			a_ep_hist.append(a_tens.tolist())
			...
			if done or (t + 1)%self.N_batch==0:
				if not done:
					...
					V_next = self.NN_value.forward(torch.tensor(s_next, dtype=torch.float))
					r_buffer[-1] += self.gamma*V_next.detach().item()
				...
				s_buffer_tens = torch.tensor(s_buffer, dtype=torch.float)
				a_buffer_tens = torch.stack(a_buffer)

				V_batch = self.NN_value.forward(s_buffer_tens).squeeze(dim=1)
				adv = r_batch_disc - V_batch

				mu_old_batch, sigma_old_batch = self.NN_policy_old.forward(s_buffer_tens)
				pi_dist_old_batch = Normal(mu_old_batch, sigma_old_batch)
				pi_old_batch = torch.exp(pi_dist_old_batch.log_prob(a_buffer_tens).sum(dim=1))

				pol_loss_batch = 0
				H_loss_batch = 0
				for epoch in range(N_epochs):

					mu_batch, sigma_batch = self.NN_policy.forward(s_buffer_tens)
					pi_dist_batch = Normal(mu_batch, sigma_batch)
					pi_batch = torch.exp(pi_dist_batch.log_prob(a_buffer_tens).sum(dim=1))

					r_prob_ratio = pi_batch/pi_old_batch


					eps_clip = 0.2
					L_clip = torch.min(	r_prob_ratio*adv.detach(),
										adv.detach()*torch.clamp(r_prob_ratio, 1 - eps_clip, 1 + eps_clip)
										)
					L_clip = L_clip.mean()
					pol_loss_batch += L_clip.detach().numpy()

					H_loss = -self.beta_entropy*(0.5 + 0.5*np.log(2*3.14) + torch.log(sigma_batch).sum(dim=1)).mean()
					H_loss_batch += H_loss.detach().numpy()

					loss_tot = L_clip + H_loss
					self.optim_policy.zero_grad()
					loss_tot.backward()
					self.optim_policy.step()


				# Update pi_old with the new pi
				self.update_old_policy_NN()
				...
				self.pol_loss_batches.append(pol_loss_batch)
				self.H_loss_batches.append(H_loss_batch)
	...
	def plot_train(self, **kwargs):
		...
		ax_R.plot(*self.smooth_data(self.R_train_hist), color='black')
	...
	def plot_trajectory(self, **kwargs):
		...
		if abs(self.last_ep_R_hist[-1] - -0.5) < 0.01:
			plt.title('Caught!', fontsize=20)
		elif abs(self.last_ep_R_hist[-1] - 1.0) < 0.01:
...
class PPO_policy(nn.Module):
	...
	def __init__(self, N_inputs, H, N_outputs, **kwargs):
		...
		nn.init.constant_(self.sigma.weight, 0.0)
	...
	def forward(self, x):
		...
		sigma = softplus(self.sigma(y)) + self.sigma_min
...
class PPO_value(nn.Module):
	...
	def __init__(self, N_inputs, H, **kwargs):
		super(PPO_value, self).__init__()

		self.critic_lin1 = nn.Linear(N_inputs, H)
		self.critic_lin2 = nn.Linear(H, H)
	...
	def forward(self, x):
		...
		y = self.critic_lin1(x)
		y = self.nonlinear(y)
		y = self.critic_lin2(y)
		y = self.nonlinear(y)
		y = self.critic_out(y)
